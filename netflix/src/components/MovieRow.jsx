...
const MovieRow = ({ title, movies }) => {
  ...
  return (
    <div className="px-[4%] relative z-20 group mb-8 scrollbar-hide ">
      ...
      <button
        onClick={() => scroll("left")}
        className="text-3xl hidden group-hover:flex items-center justify-center absolute left-4 top-32 -translate-y-1/2  w-[3.5rem] h-40 bg-black/50 hover:bg-black/70 hover:text-4xl text-white rounded-l-lg z-30 transition-all"
      >
        ❮
      </button>
      ...
      <button
        ...
        className="hidden group-hover:flex items-center justify-center absolute right-4 top-32 -translate-y-1/2 w-[3.5rem]
 h-40 bg-black/50 hover:bg-black/70 text-white rounded-r-lg z-30 transition-all duration-300 text-3xl hover:text-4xl"
      >
        ❯
      </button>
    </div>
